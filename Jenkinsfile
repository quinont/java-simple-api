            ...
            steps {
                script {
                    withCredentials([string(credentialsId: 'docker-hub-credentials', variable: 'DOCKER_HUB_CREDENTIALS')]) {
                        ...
                        sh """
                            /kaniko/executor --log-format=text --cache=false --dockerfile ${WORKSPACE}/Dockerfile --destination=${DOCKER_HUB_REPOSITORY}/java-simple-api:${VERSION} --context=${WORKSPACE}/
                        """
                    }
                }
            }
